...
- **기능 고도화 및 안정화 (3주차 - 진행 중)**
  - [x] `@ControllerAdvice`를 활용한 전역 예외 처리(Global Exception Handling)
...
#### 5. 리포지토리 정적 참조 에러 (Non-static method reference)
- **Issue**: ```IngredientRepository.existsByName(name)``` 호출 시 ```Non-static method ... cannot be referenced from a static context``` 에러가 발생하며 컴파일이 되지 않았습니다.
- **Cause**: 스프링 데이터 JPA 리포지토리는 인터페이스이며, 실제 동작은 애플리케이션 기동 시 스프링 컨테이너가 생성하는 객체(Bean) 인스턴스를 통해 이루어지는데, 이를 객체 인스턴스가 아닌 클래스 이름으로 직접 호출하여 정적 메서드와 같이 사용하려 한 것이 원인이었습니다.
- **Solution**: 클래스 레벨의 정적 호출 대신 인스턴스 호출 방식으로 수정했습니다. 해당 리포지토리를 서비스 클래스의 ```private final``` 필드로 선언하고, Lombok의 ```@RequiredArgsConstructor```를 활용해 생성자 주입을 받았습니다. 결과적으로 스프링 컨테이너가 관리하는 객체를 주입받아 안전하게 인스턴스 메서드를 호출하도록 수정했습니다.
- **배운 점**: 자바의 기초 정적 멤버와 인스턴스 멤버의 차이를 실제 프로젝트 에러를 통해 이해할 수 있었습니다. 특히 스프링 프레임워크의 의존성 주입(DI)이 단순히 코드 간결성뿐만 아니라 객체를 컨테이너가 관리함으로써 얻는 설계의 이점을 알게 되었습니다.
